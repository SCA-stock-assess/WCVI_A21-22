...
### Estimate decision:
Used PL+D as we didn't quite get a good AUC curve this year. Expanded for vis and unsurveyed sections early on (e.g., upper) as early rainfall in ~Aug likely allowed upper river access early and potential for missing SK. Type-4 because still decent coverage. 
...
### Estimate decision:
Removed heli survey as not relevant for CO. Generally good coverage and water levels for surveys, though not timed exactly for peak CO spawn (surveys end a bit early). 50-60 Coho noted in Worthless anecdotally late Oct/early Nov, but assume these were counted as part of mainstem counts. Subtracted swim-ins due to good swim coverage, assume most fish counted in the lower river were likely headed for the hatchery. Also relatively high mark rate this year (87%) suggests lots of hatchery-bound fish.
...
This means the estimate would be `r escEstSpp[escEstSpp$ShortSpecies=="CM" & escEstSpp$SystemName=="Nitinat River, Nitinat Lake",]$AdultExpAUCHighValue` and the swim-ins (`r nrh.cm %>% filter(parent_activity_type=="Adult Capture", maturity_class %in% c("Male", "Female"), grepl("Swim Ins", inventory_lot)) %>% summarize(n=sum(total_count,na.rm=T)) %>% pull(n)`) would be in addition to it (so they don't need to be accounted for here). This is because the swims mostly noted spawning Chum, and the big push came into the hatchery after the peak heli flight which was mostly spawning fish. There were also anecdotal reports of late Chum coming into Narrows after the flight. Recommended mid-Auc because expansion seemed to high otherwise. Selected the shorter AUC estimate because of hatchery anecdotal info and also to account for missed fish on flight, and that early on fish were very fungus-y and likely turned-over fast. 
...
Therefore, the final estimate for adult Chinook should be 
`r escEstSpp[escEstSpp$ShortSpecies=="CN" & escEstSpp$SystemName=="Nitinat River, Nitinat Lake",]$AdultExpAUCMidValue_R` 
minus 
`r nrh.cn %>% filter(parent_activity_type=="Adult Capture", maturity_class %in% c("Male", "Female"), grepl("Swim Ins", inventory_lot)) %>% summarize(n=sum(total_count,na.rm=T)) %>% pull(n)` swim-ins, 
plus 
`r nrh.cn %>% filter(parent_activity_type=="Adult Capture", maturity_class %in% c("Male", "Female"), operations_location=="Nitinat R") %>% summarize(n=sum(total_count,na.rm=T)) %>% pull(n)` seined fish, 
**resulting in** `r escEstSpp[escEstSpp$ShortSpecies=="CN" & escEstSpp$SystemName=="Nitinat River, Nitinat Lake",]$AdultExpAUCMidValue_R - nrh.cn %>% filter(parent_activity_type=="Adult Capture", maturity_class %in% c("Male", "Female"), grepl("Swim Ins", inventory_lot)) %>% summarize(n=sum(total_count,na.rm=T)) %>% pull(n) + nrh.cn %>% filter(parent_activity_type=="Adult Capture", maturity_class %in% c("Male", "Female"), operations_location=="Nitinat R") %>% summarize(n=sum(total_count,na.rm=T)) %>% pull(n)` **adults**

**and** 

`r escEstSpp[escEstSpp$ShortSpecies=="CN" & escEstSpp$SystemName=="Nitinat River, Nitinat Lake",]$JackExpAUCMidValue_R` 
minus 
`r nrh.cn %>% filter(parent_activity_type=="Adult Capture", maturity_class %in% c("Jack"), grepl("Swim Ins", inventory_lot)) %>% summarize(n=sum(total_count,na.rm=T)) %>% pull(n)` swim-ins, 
plus 
`r nrh.cn %>% filter(parent_activity_type=="Adult Capture", maturity_class %in% c("Jack"), operations_location=="Nitinat R") %>% summarize(n=sum(total_count,na.rm=T)) %>% pull(n)` seined fish, 
**resulting in** 
`r escEstSpp[escEstSpp$ShortSpecies=="CN" & escEstSpp$SystemName=="Nitinat River, Nitinat Lake",]$JackExpAUCMidValue_R - nrh.cn %>% filter(parent_activity_type=="Adult Capture", maturity_class %in% c("Jack"), grepl("Swim Ins", inventory_lot)) %>% summarize(n=sum(total_count, na.rm=T)) %>% pull(n) + nrh.cn %>% filter(parent_activity_type=="Adult Capture", maturity_class %in% c("Jack"), operations_location=="Nitinat R") %>% summarize(n=sum(total_count,na.rm=T)) %>% pull(n)` **jacks.**
